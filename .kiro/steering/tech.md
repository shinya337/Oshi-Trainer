...
## Reference Implementations（参考プログラム）
...
#### CameraManager.swift
- **AVFoundation活用**: カメラセッション管理の実装パターン
- **主要機能**:
  - カメラ権限の要求と状態管理
  - フロント/バックカメラの切り替え
  - ピクセルバッファのリアルタイム出力（CVPixelBuffer）
  - デリゲートパターンでのフレーム配信
  - リソースクリーンアップ管理（ResourceCleanupCoordinator）
  - セッション開始/停止の安全な制御
- **技術ポイント**:
  - `AVCaptureSession`: カメラセッション管理
  - `AVCaptureVideoDataOutput`: ビデオフレームの非同期出力
  - `@unchecked Sendable`: 並行処理の安全性確保
  - DispatchQueue活用（sessionQueue, dataOutputQueue）

#### MLModelManager.swift
- **Core ML推論管理**: 機械学習モデルの実行パターン
- **主要機能**:
  - Core MLモデルの非同期推論実行
  - バッチ処理対応
  - エラーハンドリングとフォールバック
  - パフォーマンス最適化
- **技術ポイント**:
  - `MLModel`: Core MLモデル管理
  - 非同期処理パターン（async/await）
  - リソース管理とメモリ効率化

#### ExerciseTrainingView.swift
- **UI/UX実装パターン**: トレーニング画面の構成例
- **主要機能**:
  - カメラプレビューとML推論結果のオーバーレイ表示
  - リアルタイムフィードバックUI
  - カメラとML推論の統合表示
- **技術ポイント**:
  - SwiftUI + UIViewRepresentable連携
  - リアルタイムデータバインディング

### Integration Strategy
これらの参考実装は、`realtime-form-analysis`仕様の実装時に、以下のコンポーネントへ統合される:
- `CameraService.swift`: CameraManager.swiftをベースに実装
- `PoseEstimationService.swift`: MLModelManager.swiftの推論パターンを活用
- `TrainingView.swift`: ExerciseTrainingView.swiftのUI構成を参考
