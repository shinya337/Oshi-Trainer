...
#### 🧪 実験中のプロトタイプ
- **Pythonベースの姿勢推定プロトタイプ**（複数エクササイズ対応）:
  - **共通技術スタック**:
    - YOLOv11 Poseモデルを使用したリアルタイム姿勢推定
    - ルールベースフォーム解析エンジン
    - レップカウント機能
    - 速度判定（早すぎる/遅すぎる）
    - pygame音声フィードバックシステム
  - **オーバーヘッドプレス解析**（`main_overheadpress.py`）:
    ...
    - 角度ベースのレップカウント
  - **スクワット解析**（`main_squat.py`）:
    - 膝の揺れエラー検出（Knees Inward）
    - 深さ比率（Depth Ratio）によるレップカウント
    - 音声フィードバック（knees_inward.wav, too_fast.wav, too_slow.wav）
  - **プッシュアップ解析**（`main_pushup.py`）:
    - 姿勢推定とフォーム解析
    - レップカウント機能
...
#### 📚 参考プログラム（Reference Implementations）
- **カメラ管理参考実装**（`参考プログラム/CameraManager.swift`）:
  - AVFoundationベースのカメラセッション管理
  - フロント/バックカメラ切り替え対応
  - デリゲートパターンでピクセルバッファ出力
  - リソースクリーンアップコーディネーター実装
  - リアルタイムフォーム解析機能への統合参考資料

- **ML推論管理参考実装**（`参考プログラム/MLModelManager.swift`）:
  - Core MLモデルの非同期推論実行
  - バッチ処理とパフォーマンス最適化
  - エラーハンドリングとリソース管理
  - リアルタイムフォーム解析機能への統合参考資料

- **トレーニング画面参考実装**（`参考プログラム/ExerciseTrainingView.swift`）:
  - カメラプレビューとML推論結果のオーバーレイ表示
  - UI/UXパターンの参考実装
  - リアルタイムフォーム解析機能への統合参考資料

#### 📐 設計完了フェーズ
- **リアルタイムフォーム解析（realtime-form-analysis）**:
  - Pythonプロトタイプベースのオーバーヘッドプレス姿勢解析のiOS実装
  - YOLO Poseモデルによるリアルタイムキーポイント検出
  - ルールベースフォーム判定（肘の開きエラー検出、肩幅正規化）
  - 速度判定（早すぎる/遅すぎる）とレップカウント
  - ずんだもん音声フィードバックシステム（AVAudioPlayerキュー式再生）
  - 5m離れたユーザー向けの大型UI（48pt+、レップ数64pt）
  - アーキテクチャ: 8コンポーネント（TrainingView, TrainingViewModel, CameraService, PoseEstimationService, FormAnalysisService等）
  - 現在: Design完了、Tasks生成待ち
  - 参考実装: `参考プログラム/`ディレクトリに実装例を配置
